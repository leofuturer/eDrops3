...
import { credsTaken } from '../api/user'
...
export const UserSubmitSchema = UserSchema.test('credentialsTaken', 'Username or email already taken', async ({ email, username }, ctx) =>
  credsTaken(username, email).then((res) => {
    if (res.emailTaken) return ctx.createError({ path: 'email', message: 'Email already taken' })
    if (res.usernameTaken) return ctx.createError({ path: 'username', message: 'Username already taken' })
    return true
  })
