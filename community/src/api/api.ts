...
class API {
  ...
  /**
     ...
     */
  async Request(url: string, method: string, data: object, useToken: boolean,
    headers : AxiosRequestHeaders = { 'content-type': 'application/json; charset=utf-8' }, sendDataRaw : boolean = false) {
    try {
      if (useToken) {
